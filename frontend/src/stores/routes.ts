...
export const useRouteStore = defineStore('routes', () => {
  ...
  async function setBaseline(routeId: number, traceId: number, version: number) { const { data } = await routeApi.setBaseline(routeId, traceId, version); const index = items.value.findIndex((item) => item.id === routeId); if (index >= 0) items.value[index] = data.data; return data.data }
